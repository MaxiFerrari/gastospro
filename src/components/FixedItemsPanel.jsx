...
function PendingFixedItem({ item, onFill }) {
  ...
  return (
    <div className="flex items-center gap-2 py-3 border-b border-slate-100 last:border-0">
      <div
      ...
      <input
        ...
        min="0.01"
        step="0.01"
        className="w-24 text-sm border border-slate-200 rounded-xl px-2 py-1.5 text-slate-700 placeholder-slate-300 focus:outline-none focus:ring-2 focus:ring-slate-300"
      />
      <button
        onClick={handleFill}
        disabled={saving || !amount}
        className="flex-shrink-0 p-1.5 rounded-xl bg-slate-800 text-white hover:bg-slate-700 disabled:opacity-30 disabled:pointer-events-none transition-colors"
...
function FixedItemRow({ item, onDelete }) {
  ...
  return (
    <div className="flex items-center gap-2 py-2.5 border-b border-slate-100 last:border-0">
      <div
        className={`flex-shrink-0 p-1.5 rounded-xl ${isIncome ? "bg-emerald-50" : "bg-red-50"}`}
      >
        <Icon
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-3 space-y-2">
      ...
      <input
        ...
        maxLength={120}
        className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-700 placeholder-slate-300 focus:outline-none focus:ring-2 focus:ring-slate-300"
      />
      <select
        name="category"
        value={form.category}
        onChange={handleChange}
        className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-700 bg-white focus:outline-none focus:ring-2 focus:ring-slate-300"
      >
      ...
      <button
        type="submit"
        disabled={saving}
        className="w-full py-2 rounded-xl bg-slate-800 text-white text-sm font-semibold hover:bg-slate-700 disabled:opacity-50 transition-colors flex items-center justify-center gap-2"
      >
